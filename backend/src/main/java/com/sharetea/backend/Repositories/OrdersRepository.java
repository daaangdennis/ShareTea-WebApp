...
public interface OrdersRepository extends JpaRepository<Orders, Integer> {
    ...
    @Query(value = "select order_id, order_date, first_name, last_name from orders join users on  orders.customer_id = users.user_id where is_pending = false and customer_id = ?1 order by order_date", nativeQuery = true)
