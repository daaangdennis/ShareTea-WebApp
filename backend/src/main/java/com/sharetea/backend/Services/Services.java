    ...
    public void changePermissions(Integer id, String position, String firstName, String lastName, String phoneNumber,
        ...
        if (position != null && !position.toLowerCase().equals(thisUser.getPosition().toLowerCase())) {
            ...
            try {
                ...
                HttpResponse<String> response = httpClient.send(deletePermissions,
                        HttpResponse.BodyHandlers.ofString());
                System.out.println(response);

                if (position.toLowerCase().equals("cashier") || position.toLowerCase().equals("manager")
                        || position.toLowerCase().equals("customer") || position.toLowerCase().equals("admin")) {
                    ...
                    HttpResponse<String> addResponse = httpClient.send(addPermissions,
                            HttpResponse.BodyHandlers.ofString());
                    usersRepository.save(thisUser);
    ...
    public Map<String, List<Map<String, Object>>> userOrders(HttpServletRequest request, String paramEmail)
        ...
        for (Map<String, Object> order : pendingOrders) {
            ...
            for (Map<String, Object> product : productList) {
                ...
                Map<String, Object> productNamePrice = productRepository
                        .findProductNamePrice((Integer) product.get("product_id"));
